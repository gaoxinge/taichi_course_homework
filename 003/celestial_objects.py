...
class BlackHole(CelestialObject):
    ...
    @ti.kernel
    def initialize(self, center_x: ti.f32, center_y: ti.f32, size: ti.f32, init_speed: ti.f32):
        self.pos[None] = ti.Vector([center_x, center_y])
...
class Star(CelestialObject):
    ...
    @ti.kernel
    def computeForce(self, black_hole: ti.template()):
        self.clearForce()
        for i in range(self.n):
            p = self.pos[i]
            ...
            if black_hole.Number() > 0:
                diff = black_hole.pos[None] - p
                r = diff.norm(1e-2)
                self.force[i] += G * self.Mass() * black_hole.Mass() * diff / r**3
...
class Planet(CelestialObject):
    ...
    @ti.kernel
    def computeForce(self, stars: ti.template(), black_hole: ti.template()):
        self.clearForce()
        for i in range(self.n):
            p = self.pos[i]
